...
class CMOTP(object):
    ...
    def __init__(self, version):
        ...
        if version == 3: # Stochastic
            ...
            self.c = EnvConfigV3()
        if version == 4: # Stochastic
            from .envconfig_v4 import EnvConfigV4
            self.c = EnvConfigV4()
        if version == 5: # Stochastic
            from .envconfig_v5 import EnvConfigV5
            self.c = EnvConfigV5()
        if version == 6: # Stochastic
            from .envconfig_v6 import EnvConfigV6
            self.c = EnvConfigV6()
        if version == 7: # Stochastic
            from .envconfig_v7 import EnvConfigV7
            self.c = EnvConfigV7()
        if version == 8: # Stochastic
            from .envconfig_v8 import EnvConfigV8
            self.c = EnvConfigV8()
    ...
    def stats(self):
        ...
        stats = {'Episode': str(self.episode_count), 
                 ...
                 'Indivisual_A1_Steps':str(self.individualSteps[0]),
                 'Indivisual_A2_Steps':str(self.individualSteps[1]),
                 'LeavedRoom_A1_Steps':str(self.leavedRoomSteps[0]),
                 'LeavedRoom_A2_Steps':str(self.leavedRoomSteps[1]),
    ...
    def reset(self):
        ...
        self.leavedRoom = [False for i in range(self.c.NUMBER_OF_AGENTS)]
        self.leavedRoomSteps = [0, 0]
    ...
    def step(self, actions):
        ...
        for i in range(self.c.NUMBER_OF_AGENTS):
            if self.holding_goods[i] == False:
                self.individualSteps[i] += 1

        for i in range(self.c.NUMBER_OF_AGENTS):
            if self.leavedRoom[i] == False:
                self.leavedRoomSteps[i] += 1
        
        for i in range(self.c.NUMBER_OF_AGENTS):
            if self.agents_x[i]==self.c.MID and self.agents_y[i]==self.c.GH - 3:
                self.leavedRoom[i] = True
